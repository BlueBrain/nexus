  ...
  private def executeStream(eventStream: EventStream[F]): F[Unit] = {
    ...
    val compiledStream = eventStream.value.flatMap { stream =>
      stream
        ...
        .through(printEventProgress(console, pc.errorFile))
        .evalMap {
          case (pc, tc, ev, off, org, proj) =>
            tc.queries
              .map { qc =>
                ...
                spc.query(org, proj, pc.sparqlView, query).flatMap(res => insert(qc, res)).map(_.leftMap(off -> _))
              }
              .sequence
              .map(_.sequence)
        }
        .through(printProjectionProgress(console, pc.offsetFile))
        .attempt
