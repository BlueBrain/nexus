...
object AppConfig {
  ...
  def load[F[_]](
    ...
    loadT[F](envConfigFile, postgresConfigFile, token).value
  ...
  def loadT[F[_]](
    ...
    for {
      cfg    <- assembleConfig(envConfigFile, postgresConfigFile, token)
  ...
  def assembleConfig[F[_]](
    ...
    for {
      ...
      stack        = extra withFallback overrides withFallback postgres withFallback env withFallback reference
