...
object SparqlResults {
  ...
  final case class Binding(
    ...
    /**
      * @return
      *   true when the current binding is a literal, false otherwise
      */
    def isLiteral: Boolean = `type` == "literal"

    /**
      * @return
      *   true when the current binding is an iri, false otherwise
      */
    def isIri: Boolean = `type` == "uri"

    /**
      * @return
      *   true when the current binding is a blank node, false otherwise
      */
