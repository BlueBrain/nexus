...
const rebuildSchemaWithSitePage = async ({
  ...
  const typeComposer = schemaComposer.getOTC(`SitePage`); // Clear derived types and fields
  // they will be re-created in processTypeComposer later

  (0, _derivedTypes.clearDerivedTypes)({
    schemaComposer,
    typeComposer
  });
...
const mergeTypes = ({
  ...
  if (!plugin || plugin.name === `default-site-plugin` || plugin.name === typeOwner) {
    ...
    report.warn(`Plugin \`${plugin.name}\` tried to define built-in Gatsby GraphQL type ` + `\`${typeComposer.getTypeName()}\``);
    return false;
  }
};
        ...
        Object.keys(fields).forEach(fieldName => {
          ...
          if (tc.hasField(fieldName)) {
            ...
            if (!fieldTypeName || fieldTypeName.replace(/!/g, ``) === originalTypeName.replace(/!/g, ``) || tc.getExtension(`createdFrom`) === `thirdPartySchema`) {
              ...
              tc.extendField(fieldName, newConfig); // See resetOverriddenThirdPartyTypeFields for explanation
  ...
  schemaComposer.forEach(typeComposer => {
    if (typeComposer instanceof ObjectTypeComposer || typeComposer instanceof InterfaceTypeComposer) {
      typeComposer.getFieldNames().forEach(fieldName => {
        const field = typeComposer.getField(fieldName);
        typeComposer.extendField(fieldName, {
          resolve: field.resolve ? wrappingResolver(field.resolve) : defaultResolver
        });
      });
    }
  });
...
const groupChildNodesByType = ({
  nodes
}) => _(nodes).flatMap(node => (node.children || []).map(getNode).filter(Boolean)).groupBy(node => node.internal ? node.internal.type : undefined).value();
