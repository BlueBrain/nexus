    ...
    return ts.forEachChild(ctx.sourceFile, function cb(node) {
        if (_3_0_1.isJsxElement(node)) {
            ...
            if (!missingOpeningOrClosingTag && noChildren) {
                var openingElementText = node.openingElement.getFullText();
                var selfClosingNode = openingElementText.slice(0, openingElementText.lastIndexOf(">")) + "/>";
                var fix = new Lint.Replacement(node.getStart(), node.getWidth(), selfClosingNode);
