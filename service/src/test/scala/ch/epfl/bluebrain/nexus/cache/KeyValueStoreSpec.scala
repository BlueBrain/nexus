...
import ch.epfl.bluebrain.nexus.cache.KeyValueStoreSpec.RevisionedValue
import ch.epfl.bluebrain.nexus.cache.KeyValueStoreSubscriber.KeyValueStoreChange._
import ch.epfl.bluebrain.nexus.cache.KeyValueStoreSubscriber.KeyValueStoreChanges
...
class KeyValueStoreSpec
    extends ActorSystemFixture("KeyValueStoreSpec", true, Vector(ConfigFactory.parseResources("cache/bindings.conf")))
    with Matchers
    with IOEitherValues
    with IOOptionValues
    with Eventually {
  ...
  implicit private val t: Timer[IO]         = timer(ec)
