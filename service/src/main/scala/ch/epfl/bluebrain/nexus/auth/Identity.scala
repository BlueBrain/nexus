...
sealed trait Identity extends Product with Serializable {
  def id(implicit http: HttpConfig): Uri
}
...
object Identity {
  ...
  final case object Anonymous extends Anonymous {
    def id(implicit http: HttpConfig): Uri =
      http.prefixUri.copy(path = http.prefixUri.path.?/("anonymous"))
  }
  ...
  final case class User(subject: String, realm: String) extends Subject {
    def id(implicit http: HttpConfig): Uri =
      http.realmsUri.copy(path = http.realmsUri.path / realm / "users" / subject)
  }
  ...
  final case class Group(group: String, realm: String) extends Identity {
    def id(implicit http: HttpConfig): Uri =
      http.realmsUri.copy(path = http.realmsUri.path / realm / "groups" / group)
  }
  ...
  final case class Authenticated(realm: String) extends Identity {
    def id(implicit http: HttpConfig): Uri =
      http.realmsUri.copy(path = http.realmsUri.path / realm / "authenticated")
  }
  ...
  implicit def identityEncoder(implicit http: HttpConfig): Encoder[Identity] = {
    ...
    Encoder.encodeJson.contramap { ident => enc(ident) deepMerge Json.obj("@id" -> ident.id.toString.asJson) }
  }
  ...
  def subjectIdEncoder(implicit http: HttpConfig): Encoder[Subject] =
    Encoder.encodeJson.contramap(_.id.toString.asJson)
