...
class Acls[F[_]](
  ...
  private def eval(path: Path, cmd: AclCommand): F[MetaOrRejection] =
    agg
      .evaluateS(path.toString, cmd)
      .flatMap {
        case Left(rej)         => F.pure(Left(rej))
        case Right(Initial)    => F.raiseError(UnexpectedInitialState(http.aclsUri.copy(path = http.aclsUri.path ++ path)))
  ...
  private def fetchUnsafe(path: Path): F[ResourceOpt] =
    agg.currentState(path.toString).map(stateToAcl(path, _))

  private def fetchUnsafe(path: Path, rev: Long): F[ResourceOpt] =
    agg
      .foldLeft[AclState](path.toString, Initial) {
  ...
  private def stateToAcl(path: Path, state: AclState): ResourceOpt =
    (state, path) match {
      case (Initial, p) if p == Path./ => Some(defaultResourceOnSlash)
      case (Initial, _)                => None
  ...
  private def check(path: Path, permission: Permission)(implicit caller: Caller): F[Unit] =
    hasPermission(path, permission, ancestors = true)
      .ifM(
        F.unit,
        F.raiseError(ServiceError.AccessDenied(http.aclsUri.copy(path = http.aclsUri.path ++ path), permission))
      )
