...
import ch.epfl.bluebrain.nexus.auth.Identity.Group
import ch.epfl.bluebrain.nexus.auth.{AccessToken, TokenRejection}
...
import ch.epfl.bluebrain.nexus.realms.GroupsCache.Write
...
/**
  ...
  */
class Groups[F[_]: Timer](cache: GroupsCache[F], client: HttpClient[F])(implicit cfg: GroupsConfig, F: Effect[F]) {
  ...
  implicit val logErrors: (Throwable, RetryDetails) => F[Unit] =
    (err, details) => F.pure(logger.warn(s"Retrying with details '$details'", err))
  ...
  private def fromClaimSet(claimsSet: JWTClaimsSet, realmId: RealmLabel): Set[Group] = {
  ...
  private def fetch(token: AccessToken, realm: ActiveRealm): F[Set[Group]] = {
    ...
    val req = Get(realm.userInfoEndpoint).addCredentials(OAuth2BearerToken(token.value))
    client
      ...
      .recoverWith {
        case HttpClientStatusError(_, code, body)
          ...
          F.raiseError(ServiceError.InvalidAccessToken(TokenRejection.InvalidAccessToken))
        case err: HttpClientError =>
          logger.warn(err.asString)
...
object Groups {
  ...
  /**
    ...
    */
  final def apply[F[_]: Effect: Timer](client: HttpClient[F])(
      implicit as: ActorSystem,
      cfg: GroupsConfig
  ): F[Groups[F]] =
    GroupsCache[F].map(new Groups(_, client))
}
