...
and resource representation. If you're updating from v1.0 please follow the
...
Designed primarily for applications that represent extensions to Nexus, the following service specific endpoints have
...
Additionally, a new file operation has been introduced that allows registering an existing file into the system just by
specifying its `name`, `path`, `mediaType` and optionally a `storage` identifier to be used:
...
*   `maxFileSize`: int, optional, the maximum file size allowed for clients to upload
...
*   `endpoint`: string, optional, the base address of the remote disk storage integration service, defaults to the value
    set in the environment variables of KG
*   `credentials`: string, optional, the oauth2 bearer token to be used for authenticating to the remote disk storage
    integration service, defaults to the value set in the environment variables of KG
*   `folder`: string, mandatory, the name of the folder where files are to be stored
*   `maxFileSize`: int, optional, the maximum file size allowed for clients to upload
*   `readPermission`: string, optional, defaults to `resources/read`, the permission required for downloading files from
    this storage medium
*   `writePermission`: string, optional, defaults to `files/write`, the permission required for uploading files to
...
Configuration options have been introduced such that it's consistent with the `ElasticSearchView`:
...
The implication is that triples in one namespace cannot be filtered using predicates from another nor path traversals
can be performed between the selected namespaces.
...
See @ref:[resolvers API](../api/1.1/kg/kg-resolvers-api.md) for more details.

### Paginate over 10,000
