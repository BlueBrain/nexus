...
Once the task is finished, the target project can be recreated if desired.

![deletion diagram](assets/projects/projects-delete-diagram.png)

Deletion only works when the configuration flag `app.projects.allow-resources-deletion` is set to `true`.
