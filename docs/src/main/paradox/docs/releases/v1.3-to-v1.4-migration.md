...
8. Check and update the necessary configuration environment variables/JVM properties according to @ref:[configuration migration guide](#configuration)

9. Deploy the new service images (tag `1.4`) for [delta](https://hub.docker.com/r/bluebrain/nexus-delta) and run it with the `MIGRATE_V13_TO_V14=true` and `REPAIR_FROM_MESSAGES=true` environment variables.
