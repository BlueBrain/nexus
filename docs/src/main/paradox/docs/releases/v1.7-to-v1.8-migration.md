...
in parallel. This allows for an online migration from Cassandra to Postgres that does not disrupt the users using Delta 1.7.
...
* PostgreSQL
    * Version: ^15.1
    * Storage: minimum 80% of the storage dedicated to Cassandra TODO
    * CPU: TODO
    * RAM: TODO
...
Check the total space used (in bytes) by the original `delta_1_5.messages` table:

```shell
nodetool tablestats delta_1_5.messages | grep "Space used (total)"
```
...
When ready, create a materialized view on the messages table on the keyspace `delta_1_5`:
...
### Create the PSQL tables
Run the commands from the @link:[Delta 1.8.x schema.ddl file](https://github.com/BlueBrain/nexus/tree/v1.8.x/delta/sourcing-psql/src/main/resources/scripts/schema.ddl) to create the tables necessary for Delta operations.
...
The calls must be authenticated using a service account.
