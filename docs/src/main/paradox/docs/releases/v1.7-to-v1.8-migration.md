...
2. Configure a different prefix for the new Delta 1.8 indices using the `app.defaults.indexing.prefix` field, which sets
...
A @link:[migration configuration file](https://github.com/BlueBrain/nexus/blob/$git.branch$/delta/migration/scripts/config/migration.conf) needs to be made available to Delta 1.8. Important fields are:
