...
- ElasticSearchViews no longer expand the value of `@type` field. The value used will be the one provided by the client.
- ElasticSearchViews no longer inject and expand the value of `@id` field. If `@id` is missing in the payload provided by the client, it will not be present in the Elasticsearch document.
