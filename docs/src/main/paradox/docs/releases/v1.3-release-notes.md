...
@ref:[Studio](../nexus-web/studio/index.md) (a new feature in Nexus Web) allows users to define a user interface to
browse and view the data in Nexus’ knowledge graph. Multiple @ref:[Workspaces](../nexus-web/studio/workspaces.md) can be
created within a __Studio__ allowing for arbitrary grouping of datasets to be shown. A __Workspace__ can be configured
with multiple @ref:[Dashboards](../nexus-web/studio/dashboards.md), each presenting data of possibly different types
...
In order to improve the user understanding of the linked entities of the knowledge graph, we have added a [Graph View]( https://bluebrainnexus.io/docs/nexus-web/resource-view/index.html#graph-view). It presents the outgoing entities linked and enable users to graphically navigate to them.
...
The complete configuration options can be found in the @ref:[api reference](../api/current/views/composite-view-api.md).

### View Offsets

Views now expose `offset` as a sub-resource allowing the retrieval of the last consumed event id. The event id values
are the same as the ones presented via @ref:[Server Sent Events](../api/current/kg-resources-api.md#resources-server-sent-events).
...
The SparqlViews can now also @ref:[be queried](../api/current/views/sparql-view-api.md#sparql-query) using the GET
