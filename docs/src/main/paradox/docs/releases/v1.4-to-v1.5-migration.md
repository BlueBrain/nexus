...
Nonetheless, migration does not require to stop an interruption of service of Delta 1.4 to be performed, so a double-run of delta instances is recommended during this period.

**Please do not perform any write operations as they may have an impact on the ongoing migration.**

## Requirements
As migration require having two coexisting keyspaces, the following resources are recommended:

* Disk space for Cassandra: 3x more disk space than the amount 1.4 is using (as we now store the compacted/expanded form of the resources)
...
The previous Delta instance can be stopped a complete reindexing of the data in elasticsearch/blazegraph will take place. 
...
* As a entire indexing will take place, some timeouts may occur
