...
Since storages can no longer be created with credentials, it is recommended that they should be configured as follows:
```hocon
remote-disk {
  credentials {
...
If you wish to specify a long-lived access token (the legacy method), then the configuration would look like this:
```hocon
remote-disk {
  credentials {
    type: "jwt-token"
    token: "delta-long-lived-access-token"
  }
}
```

You can also specify to log in anonymously:
```hocon
remote-disk {
  credentials {
    type: "anonymous"
