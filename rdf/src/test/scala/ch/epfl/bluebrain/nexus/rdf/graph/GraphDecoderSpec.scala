...
class GraphDecoderSpec extends RdfSpec with JenaSpec {
  ...
  "A Decoder" should {
    ...
    "successfully decode rdf types" in {
      c.as[Uri].rightValue shouldEqual id
      c.down(nxv / "url").as[Uri].rightValue shouldEqual id
      c.down(nxv / "idUrl").as[Uri].rightValue shouldEqual id
    ...
    "fail to decode" in {
      ...
      c.down(nxv / "float").as[Uri].leftValue
