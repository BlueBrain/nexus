...
import ch.epfl.bluebrain.nexus.rdf.graph.GraphEncoderSpec.IntValue
...
class GraphEncoderSpec extends RdfSpec with JenaSpec {
  ...
  "An Encoder" should {
    ...
    "correctly encode Uris" in {
      GraphEncoder[Uri].apply(schema.value) shouldEqual Graph(schema.value)
    }
...
object GraphEncoderSpec {
  final case class IntValue(id: Uri, value: Int)
  object IntValue {
