...
import ch.epfl.bluebrain.nexus.rdf.graph.Graph.{OptionalGraph, SetGraph, Triple}
...
class GraphSpec extends RdfSpec {
  ...
  "A Graph" should {
    ...
    "return the correct ntriples representation" in {
      ...
      val expectedLines =
        Set(
          ...
          """<http://nexus.example.com/other> <http://schema.org/birthHour> "9"^^<http://www.w3.org/2001/XMLSchema#int> .""",
          """<http://nexus.example.com/john-doe> <http://schema.org/birthDate> "1999-04-09T20:00Z" .""",
          """<http://nexus.example.com/other> <http://schema.org/birthYear> "1999"^^<http://www.w3.org/2001/XMLSchema#int> ."""
        )

      val ntriples = g.ntriples
      ntriples.split("\n").toSet shouldEqual expectedLines
    }
    ...
    "return the correct DOT representation with prefix mappings" in {
      val jDoe  = url"http://nexus.example.com/john-doe"
      val other = url"http://nexus.example.com/other"
      val mappings: Map[Uri, String] = Map(
