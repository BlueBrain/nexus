...
import ch.epfl.bluebrain.nexus.rdf.graph.Graph.{OptionalGraph, SetGraph}
...
trait RdfEncoderInstances {
  import GraphEncoder.instance
  implicit final val graphEncodeUri: GraphEncoder[Uri] = instance(value => Graph(IriNode(value)))
}
