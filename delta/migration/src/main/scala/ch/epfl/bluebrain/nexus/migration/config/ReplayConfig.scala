package ch.epfl.bluebrain.nexus.migration.config

import ch.epfl.bluebrain.nexus.migration.replay.{BucketSize, TimeBucket}
...
object ReplayConfig {
  ...
  /**
    ...
    */
  def from(config: Config): ReplayConfig = {
    ...
    ReplayConfig(keyspace, bucketSize, firstTimeBucket, refreshInterval, eventualConsistency, maxBufferSize)
  }
}
