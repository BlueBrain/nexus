                         ...
                         .map { envelope =>
                           envelope.value match {
                             case f if f.storageType == StorageType.RemoteDiskStorage && !f.attributes.digest.computed =>
                               SuccessElem(entityType, envelope.id, Some(envelope.value.project), envelope.instant, envelope.offset, f, envelope.rev)
                             case _                                                                                    =>
                               DroppedElem(entityType, envelope.id, Some(envelope.value.project), envelope.instant, envelope.offset, envelope.rev)
                           }
                         }
                         .evalMap {
                           _.traverse { f =>
