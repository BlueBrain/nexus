...
class FilesRoutesSpec
  ...
  private val (files, storages) = FilesSetup.init(orgs, projs, acls, stCfg, ConsistentWriteDummy())
  ...
  "File routes" should {
    ...
    "create storages for files" in {
      ...
      acls
        ...
        .accepted
      storages.create(s3Id, projectRef, diskFieldsJson.map(_ deepMerge defaults deepMerge s3Perms), Performant).accepted
      storages.create(dId, projectRef, diskFieldsJson.map(_ deepMerge defaults), Performant).accepted
    }
