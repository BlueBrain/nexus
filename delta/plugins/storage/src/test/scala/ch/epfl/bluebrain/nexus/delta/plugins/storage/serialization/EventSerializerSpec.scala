...
class EventSerializerSpec
  ...
  val filesMapping: Map[FileEvent, Json] = VectorMap(
    FileCreated(
      ...
      instant,
      subject
    )                                                                                             -> jsonContentOf("/file/serialization/file-created.json"),
    FileUpdated(fileId, projectRef, storageRef, DiskStorageType, attributes, 2, instant, subject) -> jsonContentOf(
      "/file/serialization/file-updated.json"
    ),
    FileAttributesUpdated(
      ...
      instant,
      subject
    )                                                                                             -> jsonContentOf("/file/serialization/file-attributes-created-updated.json"),
    FileTagAdded(fileId, projectRef, targetRev = 1, tag, 4, instant, subject)                     -> jsonContentOf(
      "/file/serialization/file-tag-added.json"
    ),
    FileTagDeleted(fileId, projectRef, tag, 4, instant, subject)                                  -> jsonContentOf(
      "/file/serialization/file-tag-deleted.json"
    ),
