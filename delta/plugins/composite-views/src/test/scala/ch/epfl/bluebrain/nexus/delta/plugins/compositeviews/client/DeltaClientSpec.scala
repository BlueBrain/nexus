...
class DeltaClientSpec
  ...
  "Getting project statistics" should {
    ...
    "work" in {
      deltaClient.projectCount(source).accepted shouldEqual ProjectStatistics(10, 10, Instant.EPOCH)
    }
  ...
  "Getting events" should {
    "work" in {
      val stream = deltaClient.events[Int](source, Offset.Start)

      val expected = (0 to 4).map { i =>
        Offset.at(i.toLong) -> i
      }
