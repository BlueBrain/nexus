...
import ch.epfl.bluebrain.nexus.delta.sourcing.model.{AllowedViewTypes, Identity, Label, ProjectRef}
...
class CompositeViewFactorySuite extends NexusSuite {
  ...
  private val schemas: AllowedViewTypes = AllowedViewTypes.fromIri(nxv + "Schema")
  private val types: AllowedViewTypes   = AllowedViewTypes.fromIri(nxv + "Type")
  ...
  test("Preserve the uuid and update the indexing rev when the projection has changed.") {
    ...
    val current       = SparqlProjection(
      ...
      blazegraphProjectionFields.query,
      schemas,
      AllowedViewTypes.fromIri(nxv + "OldType"),
      includeMetadata,
      includeDeprecated,
      permissions.query
    )
