...
import ch.epfl.bluebrain.nexus.delta.sourcing.model.{ElemStream, ViewRestriction}
...
object DeltaClient {
  ...
  final private class DeltaClientImpl(
    ...
    private def typeQuery(restriction: ViewRestriction) = restriction match {
      case ViewRestriction.None                => Query.Empty
      case ViewRestriction.RestrictedTo(types) => Query(types.map(t => "type" -> t.toString).toList: _*)
    }
