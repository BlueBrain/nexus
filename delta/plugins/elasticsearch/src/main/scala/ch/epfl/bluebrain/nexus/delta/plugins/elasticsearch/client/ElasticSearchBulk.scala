...
object ElasticSearchBulk {
  ...
  final case class Index(index: IndexLabel, id: String, content: JsonObject)  extends ElasticSearchBulk {
    def payload: String = Json.obj("index" -> json).noSpaces + newLine + content.asJson.noSpaces
  }
  final case class Create(index: IndexLabel, id: String, content: JsonObject) extends ElasticSearchBulk {
    def payload: String = Json.obj("create" -> json).noSpaces + newLine + content.asJson.noSpaces
  }
  final case class Update(index: IndexLabel, id: String, content: JsonObject, retry: Int = 0)
      extends ElasticSearchBulk {
