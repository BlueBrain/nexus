...
/**
  ...
  */
class ElasticSearchClient(client: HttpClient, endpoint: Uri, maxIndexPathLength: Int, elasticSearchEmptyResults: Json)(
    implicit
  ...
  def search(
    ...
    if (indices.isEmpty) IO.pure(elasticSearchEmptyResults)
    else {
      val (indexPath, q) = indexPathAndQuery(indices, QueryBuilder(query))
