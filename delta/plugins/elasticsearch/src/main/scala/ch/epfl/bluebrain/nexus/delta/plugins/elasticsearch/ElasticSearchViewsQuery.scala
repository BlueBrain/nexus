...
final class ElasticSearchViewsQueryImpl private[elasticsearch] (
  ...
  def query(
    ...
    for {
      ...
      indices <- view match {
                   case v: IndexingView  =>
                     aclCheck.authorizeForOr(v.ref.project, v.permission)(AuthorizationFailed).as(Set(v.index)).toBIO[AuthorizationFailed]
  ...
  override def mapping(
    ...
    for {
      view   <- aclCheck.authorizeForOr(project, permissions.write)(AuthorizationFailed).as(viewStore.fetch(id, project)).toBIO[AuthorizationFailed]
      index  <- view.map {
                  case v: IndexingView  =>
                    IO.pure(v.index)
