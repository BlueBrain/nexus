...
trait RemoteContextResolutionFixture {
  implicit val rcr: RemoteContextResolution = RemoteContextResolution.fixed(
    elasticsearch                  -> jsonContentOf("contexts/elasticsearch.json").topContextValueOrEmpty,
    contexts.elasticsearchIndexing -> jsonContentOf("/contexts/elasticsearch-indexing.json").topContextValueOrEmpty,
