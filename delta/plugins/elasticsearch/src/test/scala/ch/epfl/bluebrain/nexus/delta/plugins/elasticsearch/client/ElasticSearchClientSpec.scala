...
class ElasticSearchClientSpec
  ...
  private def searchAllIn(index: String): Seq[JsonObject] =
    client.search(JsonObject.empty, Set(index))(page).accepted.sources

  "An ElasticSearch Client" should {

    "fetch the service description" in {
      client.serviceDescription.accepted shouldEqual ServiceDescription("7.10.2")
    }

    "verify that an index does not exist" in {
      client.existsIndex("some").accepted shouldEqual false
    }

    "create index" in {
      client.createIndex("some").accepted shouldEqual true
    }

    "attempt to create index a second time" in {
      client.createIndex("some").accepted shouldEqual false
    }

    "attempt to create an index with wrong payload" in {
      client.createIndex("other", jobj"""{"key": "value"}""").rejectedWith[HttpClientStatusError]
    }

    "delete an index" in {
      client.createIndex("other").accepted
      client.deleteIndex("other").accepted shouldEqual true
    }

    "attempt to delete a non existing index" in {
      client.deleteIndex("other").accepted shouldEqual false
    }
    ...
    "search" in {
      ...
      eventually {
        client.search(query, Set(index))(page, sort = SortList(List(Sort("-field1")))).accepted shouldEqual
          SearchResults(2, Vector(jobj"""{ "field1" : 3 }""", jobj"""{ "field1" : 1, "field2" : "value2"}"""))
            .copy(token = Some("[1]"))
      }

      val query2 = jobj"""{"query": {"bool": {"must": {"term": {"field1": 3} } } } }"""
      client.search(query2, Set(index))(page).accepted shouldEqual
    ...
    "search returning raw results" in {
      ...
      val query2     = jobj"""{"query": {"bool": {"must": {"term": {"field1": 3} } } } }"""
      eventually {
        client.searchRaw(query2, Set(index))(page).accepted.removeKeys("took") shouldEqual
