...
import ch.epfl.bluebrain.nexus.delta.plugins.elasticsearch.model.{permissions, ElasticSearchViewRejection, IndexingViewResource}
...
import ch.epfl.bluebrain.nexus.delta.sdk.{JsonLdValue, ProgressesStatistics}
...
class ElasticSearchIndexingSpec(override val docker: ElasticSearchDocker)
  ...
  private val messages =
    ...
    ).zipWithIndex.foldLeft(Map.empty[(ProjectRef, Option[UserTag]), Seq[Message[EventExchangeResult]]]) {
  ...
  private lazy val views: ElasticSearchViews = ElasticSearchViewsSetup.init(fetchContext, permissions.query)
