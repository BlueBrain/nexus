...
class ElasticSearchIndexingEventLogSpec
  ...
  implicit private val projectionId: ProjectionId = ViewProjectionId("elasticsearch-projection")
  ...
  private val globalEventLog = ElasticSearchIndexingEventLog(
    journal.asInstanceOf[EventLog[Envelope[Event]]],
    Set(new ResourceEventExchangeDummy(resources)),
    2,
    150.millis
  )
  private val tpe            = nxv + "Tpe"
  private val name1          = "myName"
  private val name2          = "myName2"
  ...
  private val source        =
    jsonContentOf("indexing/resource.json", "id" -> myId, "type" -> tpe, "number" -> 10, "name" -> name1)
  private val sourceUpdated = source deepMerge Json.obj("number" -> Json.fromInt(42))
  private val source2       =
    jsonContentOf("indexing/resource.json", "id" -> myId2, "type" -> tpe, "number" -> 20, "name" -> name2)
  ...
  private def toIndexData(res: Resource, name: String) = {
  ...
  private val allEvents =
    List(
      Chunk(
        ...
        SuccessMessage(Sequence(2), r1Updated.updatedAt, resourceId(r1Updated.id, projectRef), 2, r1Updated.map(toIndexData(_, name1)), Vector.empty)
      ),
      Chunk(
        SuccessMessage(Sequence(3), r2Created.updatedAt, resourceId(r2Created.id, project2Ref), 1, r2Created.map(toIndexData(_, name2)), Vector.empty)
      )
    )
  // format: on

  "An ElasticSearchIndexingEventLog" should {
