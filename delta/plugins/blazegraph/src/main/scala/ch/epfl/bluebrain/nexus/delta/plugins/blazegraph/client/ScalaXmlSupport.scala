 ...
 * Ported from akka.http.scaladsl.marshallers.xml.ScalaXmlSupport
 * To be removed once this PR is merged: https://github.com/akka/akka-http/pull/3804
 */
// $COVERAGE-OFF$
...
object ScalaXmlSupport extends ScalaXmlSupport {
  val nodeSeqMediaTypes: immutable.Seq[MediaType.NonBinary]     = List(
    ...
    RdfMediaTypes.`application/sparql-results+xml`
  )
  ...
  def createSaferSAXParser(): SAXParser = {
    ...
    val _      = Try { parser.setProperty("http://apache.org/xml/properties/locale", java.util.Locale.ROOT) }
    parser
  }
}
// $COVERAGE-ON$
