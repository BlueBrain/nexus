...
class IndexingViewDefSuite extends NexusSuite {
  ...
  private val filterByTypeConfig = FilterByTypeConfig(ValidViewTypes.restrictedTo(nxv + "PullRequest"))
  private val indexing           = IndexingBlazegraphViewValue(
    resourceTag = Some(UserTag.unsafe("some.tag")),
    resourceTypes = ValidViewTypes.restrictedTo(nxv + "PullRequest")
  )
