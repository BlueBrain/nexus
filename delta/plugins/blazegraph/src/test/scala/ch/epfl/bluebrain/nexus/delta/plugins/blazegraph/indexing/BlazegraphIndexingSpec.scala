...
class BlazegraphIndexingSpec
  ...
  private val views: BlazegraphViews = BlazegraphViewsSetup.init(orgs, projs, ConsistentWriteDummy(), permissions.query)
  ...
  "BlazegraphIndexing" should {
    ...
    "index resources for project1" in {
      val view = views.create(viewId, project1.ref, indexingValue, Performant).accepted.asInstanceOf[IndexingViewResource]
    ...
    "index resources for project2" in {
      val view = views.create(viewId, project2.ref, indexingValue, Performant).accepted.asInstanceOf[IndexingViewResource]
      checkBlazegraphTriples(
        view,
    ...
    "index resources with metadata" in {
      ...
      val project1View = views.update(viewId, project1.ref, 1L, indexVal, Performant).accepted.asInstanceOf[IndexingViewResource]
      checkBlazegraphTriples(
        project1View,
    ...
    "index resources including deprecated" in {
      ...
      val view     = views.update(viewId, project1.ref, 2L, indexVal, Performant).accepted.asInstanceOf[IndexingViewResource]
      checkBlazegraphTriples(
        view,
        triplesFor(res2Proj1, value2Proj1),
    ...
    "index resources constrained by schema" in {
      ...
      val view     = views.update(viewId, project1.ref, 3L, indexVal, Performant).accepted.asInstanceOf[IndexingViewResource]
      checkBlazegraphTriples(
        view,
        triplesFor(res3Proj1, value3Proj1),
        triplesFor(res1rev2Proj1, value1rev2Proj1)
    ...
    "index resources with type" in {
      ...
      val view     = views.update(viewId, project1.ref, 4L, indexVal, Performant).accepted.asInstanceOf[IndexingViewResource]
