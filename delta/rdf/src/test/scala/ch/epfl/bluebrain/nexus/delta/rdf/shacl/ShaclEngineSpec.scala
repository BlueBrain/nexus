...
class ShaclEngineSpec
  ...
  "A ShaclEngine" should {
    ...
    "validate data from schema shapes" in {
      val report = ShaclEngine(resourceGraph, schemaShapes, reportDetails = true, validateShapes = false).accepted
      report.isValid() shouldEqual true
    }

    "validate shapes" in {
      ShaclEngine(schemaGraph, reportDetails = true).accepted.isValid() shouldEqual true
    }
