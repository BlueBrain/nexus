...
/**
  ...
  */
class ApplyOwnerPermissionsDummy private (
    acls: Acls,
...
object ApplyOwnerPermissionsDummy {

  /**
    * Create a new instance of [[ApplyOwnerPermissionsDummy]]
    *
    * @param acls             an acls instance
    * @param ownerPermissions the owner permissions to be present on the project/org
    * @param serviceAccount   the service account to apply the acls if needed
    */
  def apply(
      acls: Acls,
      ownerPermissions: Set[Permission],
      serviceAccount: Identity.Subject
  ): ApplyOwnerPermissionsDummy = new ApplyOwnerPermissionsDummy(acls, ownerPermissions, serviceAccount)
}
