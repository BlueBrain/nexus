...
import ch.epfl.bluebrain.nexus.delta.sdk.testkit.{ConsistentWriteDummy, ProjectSetup, ResolversDummy}
...
class ResolverReferenceExchangeSpec
  ...
  private val resolvers: Resolvers = ResolversDummy(orgs, projs, resolverContextResolution, (_, _) => IO.unit, ConsistentWriteDummy()).accepted
  ...
  "A ResolverReferenceExchange" should {
    ...
    val resRev1 = resolvers.create(id, project.ref, source, Performant).accepted
    val resRev2 = resolvers.tag(id, project.ref, tag, 1L, 1L, Performant).accepted
