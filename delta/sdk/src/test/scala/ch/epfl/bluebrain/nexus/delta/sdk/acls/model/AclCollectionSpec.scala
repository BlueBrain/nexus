...
class AclCollectionSpec extends AnyWordSpecLike with Matchers with AclFixtures {
  ...
  "A Collection of ACL" should {
    ...
    def acl(address: AclAddress): AclResource  = AclGen.resourceFor(userRW_groupX(address), 1, subject)
    def acl2(address: AclAddress): AclResource = AclGen.resourceFor(groupR(address), 2, subject)
    def acl3(address: AclAddress): AclResource = AclGen.resourceFor(groupX(address), 3, subject)
