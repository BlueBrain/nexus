...
class UriDirectivesSpec extends BaseSpec with RouteHelpers with UriDirectives with Inspectors {
  ...
  "A route" should {
    ...
    "return before if an end of range is provided" in {
      val end     = Instant.now()
      val encoded = encodeUriQuery(end.toString)
    ...
    "return after if an start of range is provided" in {
      ...
      val encoded = encodeUriQuery(start.toString)
      Get(s"/base/timerange?createdAt=$encoded..*") ~> Accept(`*/*`) ~> route ~> check {
    ...
    "return between is an start of range is provided" in {
      ...
      val encodedStart = encodeUriQuery(start.toString)
    ...
    "return an IriSegment" in {
      ...
      val encoded = encodeUriPath(iri.toString)
    ...
    "reject on invalid search parameters" in {
      val group     = encodeUriQuery(Group("mygroup", Label.unsafe("myrealm")).asIri.toString)
      val endpoints = List(
