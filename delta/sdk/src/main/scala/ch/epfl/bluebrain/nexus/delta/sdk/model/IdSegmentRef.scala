...
sealed trait IdSegmentRef extends Product with Serializable { self =>
  ...
  /**
    ...
    */
  def toRev(rev: Int): IdSegmentRef.Revision = IdSegmentRef.Revision(self.value, rev)
...
object IdSegmentRef {
  ...
  def fromRevOpt(id: IdSegment, revOpt: Option[Int]): IdSegmentRef     = revOpt.fold(apply(id))(Revision(id, _))
  ...
  /**
    ...
    */
  final case class Revision(value: IdSegment, rev: Int) extends IdSegmentRef
