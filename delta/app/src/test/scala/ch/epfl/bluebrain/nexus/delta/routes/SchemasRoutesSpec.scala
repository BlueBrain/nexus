...
class SchemasRoutesSpec extends BaseRouteSpec with IOFromMap with CatsIOValues {
      ...
      forAll(endpoints) { endpoint =>
        Get(endpoint) ~> asReader ~> routes ~> check {
          ...
          response.asJson shouldEqual jsonContentOf(
            ...
            "id"   -> "nxv:myid",
            "self" -> ResourceScopeF.schema(projectRef, myId).accessUri
          )
        }
      }
    }
        ...
        forAll(List("rev=1", "tag=mytag")) { param =>
          Get(s"$endpoint?$param") ~> asReader ~> routes ~> check {
            ...
            response.asJson shouldEqual jsonContentOf(
              ...
              "id"   -> "nxv:myid2",
              "self" -> ResourceScopeF.schema(projectRef, myId2).accessUri
            )
  ...
  private def schemaMetadata(
    ...
    jsonContentOf(
      ...
      "self"       -> ResourceScopeF.schema(project, id).accessUri
    )
}
