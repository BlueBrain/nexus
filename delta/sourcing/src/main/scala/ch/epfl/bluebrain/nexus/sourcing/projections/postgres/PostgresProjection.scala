...
private[projections] class PostgresProjection[A: Encoder: Decoder](
  ...
  override def recordProgress(id: ProjectionId, progress: ProjectionProgress): Task[Unit] =
    instant.flatMap { timestamp =>
      sql"""INSERT into projections_progress(projection_id, akka_offset, timestamp, processed, discarded, warnings, failed)
           |VALUES(${id.value}, ${offsetToSequence(progress.offset)}, ${timestamp.toEpochMilli}, ${progress.processed},
           |${progress.discarded}, ${progress.warnings}, ${progress.failed})
           |ON CONFLICT (projection_id) DO UPDATE SET akka_offset=EXCLUDED.akka_offset, timestamp=EXCLUDED.timestamp,
           |processed=EXCLUDED.processed, discarded=EXCLUDED.discarded, warnings=EXCLUDED.warnings, failed=EXCLUDED.failed""".stripMargin.update.run
        .transact(xa)
        .map(_ => ())
    }
  ...
  override def progress(id: ProjectionId): Task[ProjectionProgress] =
    sql"SELECT akka_offset, timestamp, processed, discarded, warnings, failed FROM projections_progress WHERE projection_id = ${id.value}"
      .query[(Option[Long], Long, Long, Long, Long, Long)]
      .option
      .transact(xa)
      .map {
        _.fold(NoProgress) { case (offset, timestamp, processed, discarded, warnings, failed) =>
          ProjectionProgress(
