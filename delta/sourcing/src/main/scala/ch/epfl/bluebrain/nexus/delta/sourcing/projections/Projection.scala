...
import ch.epfl.bluebrain.nexus.delta.sourcing.config.{DatabaseConfigOld, PostgresConfig}
...
/**
  ...
  * In terms of CQRS, the events represents the format in which data is written to the primary store (the write model)
  * while the result of a projection represents the data in a consumable format (the read model).
  *
...
trait Projection[A] {
  ...
  /**
    ...
    *   the projection identifier
    * @param messages
    *   the messages with potentially errors to save
    */
  def recordErrors(
      id: ProjectionId,
...
object Projection {
  ...
  /**
    ...
    * @return
    *   the stacktrace formatted in an human readable fashion
    */
  ...
  def apply[A: Decoder: Encoder](
    ...
    config.flavour match {
      ...
      case Cassandra => Projection.postgres(config.postgres, empty)
    }
  }
  ...
  /**
    * A Projection that records its progress in memory. This implementation does not survive system restarts.
    */
