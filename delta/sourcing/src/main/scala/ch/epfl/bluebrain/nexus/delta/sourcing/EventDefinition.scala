...
import ch.epfl.bluebrain.nexus.delta.sourcing.processor.StopStrategy
...
sealed trait EventDefinition[State, Command, Event, Rejection] extends Product with Serializable {
  ...
  /**
    * State transition function; represented as a total function without any effect types; Should be pure
    */
  def next: (State, Event) => State

  /**
    * Command evaluation function; represented as a function that returns the evaluation in an arbitrary effect type May
    * be asynchronous
    */
...
/**
  * Implementation of an [[EventDefinition]] relying on akka-persistence
  * @param tagger
  *   the tags to apply to the event
  * @param snapshotStrategy
  *   the snapshot strategy to apply
  * @param stopStrategy
  *   the stop strategy to apply
  */
...
object TransientEventDefinition {
  ...
  /**
    * Create a transient definition which describes a cache, where evaluation results directly in a new state which
