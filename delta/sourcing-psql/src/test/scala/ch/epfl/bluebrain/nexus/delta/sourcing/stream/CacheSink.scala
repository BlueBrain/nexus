...
import ch.epfl.bluebrain.nexus.delta.sourcing.stream.Operation.{Sink, SinkCatsEffect}
import fs2.Chunk
...
final class CacheSink[A: Typeable] private (documentId: Elem[A] => Iri) extends SinkCatsEffect {
...
final class CacheSink2[A: Typeable] private (documentId: Elem[A] => Iri) extends Sink {
...
object CacheSink2 {
  ...
  /** CacheSink for events */
  def events[A: Typeable]: CacheSink2[A] = new CacheSink2(eventDocumentId)

  def states[A: Typeable]: CacheSink2[A] = new CacheSink2(_.id)
}
