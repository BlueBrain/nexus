...
/**
  ...
  * @param readConfig the hikari config for database reads but streaming queries
  * @param writeConfig the hikari config for database writes
  * @param streamingConfig the hikari config for streaming queries
  * @param tablesAutocreate allows to create automatically tables at startup
  * @param query the configuration for streaming queries
  */
...
object SourcingConfig {
  ...
  /**
    ...
    * @param batchSize the maximum number of elements to fetch with one query
    * @param refreshInterval the time to wait before reexecuting the query when the rows have all been consumed
    */
