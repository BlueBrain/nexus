...
class BatchCopySpec extends BaseIntegrationSpec {
  ...
  def uploadFile(file: FileInput, storage: StorageDetails): IO[Assertion] =
    filesDsl
      .uploadFileWithMetadata(file, storage.projRef, storage.storageId, None, file.keywords, Some(file.description))
  ...
  def checkFileContentsAreCopiedCorrectly(destProjRef: String, sourceFiles: List[FileInput], ids: List[String]) =
    ids.zip(sourceFiles).traverse { case (destId, FileInput(_, filename, contentType, contents, _, _)) =>
      deltaClient
  ...
  def assertFileUserMetadataWasCopiedCorrectly(
    ...
    ids
      .zip(sourceFiles)
      .parTraverse { case (id, FileInput(_, _, _, _, keywords, description)) =>
