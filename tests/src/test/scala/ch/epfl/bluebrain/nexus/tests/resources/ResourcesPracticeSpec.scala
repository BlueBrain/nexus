...
class ResourcesPracticeSpec extends BaseSpec with CirceEq {
  ...
  "Generating resources" should {
    ...
    "fail for a user without access" in {
      deltaClient.getWithBody[Json](s"/practice/resources/$ref/", payloadWithoutSchema, Alice)(expectForbidden)
    }

    "fail for an unknown project" in {
      deltaClient.getWithBody[Json](s"/practice/resources/$org/xxx/", payloadWithoutSchema, Alice)(expectForbidden)
    }

    "succeed for a payload without schema" in {
      deltaClient.getWithBody[Json](s"/practice/resources/$ref/", payloadWithoutSchema, Bob) { (json, response) =>
    ...
    "succeed for a payload with an existing schema" in {
      deltaClient.getWithBody[Json](s"/practice/resources/$ref/", payloadWithExistingSchema, Bob) { (json, response) =>
        response.status shouldEqual StatusCodes.OK
        resultId(json).value shouldEqual resourceId
        schema(json) shouldBe empty
        errorType(json) shouldBe empty
    ...
    "succeed for a payload with a new schema" in {
      deltaClient.getWithBody[Json](s"/practice/resources/$ref/", payloadWithNewSchema, Bob) { (json, response) =>
        response.status shouldEqual StatusCodes.OK
        resultId(json).value shouldEqual resourceId
    ...
    "fail for a resource with an invalid context without generating any schema" in {
      ...
      deltaClient.getWithBody[Json](s"/practice/resources/$ref/", payload, Bob) { (json, response) =>
        response.status shouldEqual StatusCodes.OK
